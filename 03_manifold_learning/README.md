...
├── mnist_manifold_3d.ipynb       # MNIST: 4가지 기법 3D 인터랙티브 비교
├── swiss_roll_manifold.ipynb     # Swiss Roll: 언폴딩 + 파라미터 민감도 분석
├── *.html                        # 인터랙티브 시각화 결과 (실행 후 생성)
└── README.md
```

### swiss_roll_manifold.ipynb 주요 내용
- 각 기법으로 Swiss Roll을 어떻게 펼치는지 직접 비교
- 하이퍼파라미터 민감도 분석 (Isomap k / LLE method & k / t-SNE perplexity / UMAP n_neighbors & min_dist)
- 계산 속도 벤치마크
- 이웃 t값 분산 · Trustworthiness · 실루엣 스코어 정량 평가
